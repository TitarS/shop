...
class ModelToolCtrlImage extends Model
{
    ...
    public function resize($filename, $width, $height, $type = "")
    {
        ...
        $watermark = $this->config->get('ctrl_watermark_status');
        if (!$watermark) {
            $this->debug("Watermark is disabled");
        }

        if($watermark) {
            $watermark_image = $this->config->get('ctrl_watermark_image');
            if (!$watermark_image ) {
                $this->debug("Watermark image is not selected");
                $watermark = FALSE;
        ...
        if($watermark) {
            $watermark_min_height = $this->config->get('ctrl_watermark_min_width');
            $watermark_min_width = $this->config->get('ctrl_watermark_min_height');
        ...
        if($watermark) {
            $watermark_max_height = $this->config->get('ctrl_watermark_max_width');
            $watermark_max_width = $this->config->get('ctrl_watermark_max_height');
        ...
        if($watermark) {
            $exclude = $this->config->get('ctrl_watermark_exclude');
            if(is_array($exclude)) {
        ...
        $watermark_width = $this->config->get("ctrl_watermark_width");
        $watermark_height = $this->config->get("ctrl_watermark_height");
        $watermark_left = $this->config->get("ctrl_watermark_left");
        $watermark_top = $this->config->get("ctrl_watermark_top");
        $watermark_angle = $this->config->get("ctrl_watermark_angle");
        $watermark_hide_real_path = $this->config->get("ctrl_watermark_hide_real_path");
        ...
        if (!file_exists(DIR_IMAGE . $new_image) || (filemtime(DIR_IMAGE . $old_image) > filemtime(DIR_IMAGE . $new_image))) {
            ...
            if ($width_orig != $width || $height_orig != $height || $watermark) {
                ...
                if ($watermark) {
                    $this->debug("watermark was not called for unknown reason");
                }
